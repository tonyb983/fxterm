...
const val GET_NOTE_BY_ID: String =
    "query GetNoteById(${'$'}id: UUID!) {\r\n  getById(id:${'$'}id) {\r\n    id\r\n    title\r\n    content\r\n    tags\r\n    created\r\n    updated\r\n  }\r\n}\r\n\r\n"

class GetNoteById(
  private val graphQLClient: GraphQLWebClient
) {
  suspend fun execute(variables: GetNoteById.Variables,
      requestBuilder: WebClient.RequestBodyUriSpec.() -> Unit = {}):
      GraphQLResponse<GetNoteById.Result> = graphQLClient.execute(GET_NOTE_BY_ID, "GetNoteById",
      variables, requestBuilder)

  data class Variables(
    val id: GetNoteById.UUID
  )
  ...
  data class Note(
     ...
     */
    val created: GetNoteById.LocalDateTime,
    /**
     * The LocalDateTime of when the Note was last updated.
     */
    val updated: GetNoteById.LocalDateTime
  )

  data class Result(
    val getById: GetNoteById.Note?
  )
}
