...
const val SEARCH_NOTES_BY_CONTENT: String =
...
class SearchNotesByContent(
  private val graphQLClient: GraphQLWebClient
) {
  suspend fun execute(variables: SearchNotesByContent.Variables,
      requestBuilder: WebClient.RequestBodyUriSpec.() -> Unit = {}):
      GraphQLResponse<SearchNotesByContent.Result> = graphQLClient.execute(SEARCH_NOTES_BY_CONTENT,
      "SearchNotesByContent", variables, requestBuilder)
  ...
  data class Note(
     ...
     */
    val id: SearchNotesByContent.UUID,
    /**
     ...
     */
    val created: SearchNotesByContent.LocalDateTime,
    /**
     * The LocalDateTime of when the Note was last updated.
     */
    val updated: SearchNotesByContent.LocalDateTime
  )

  data class Result(
    val findByContent: List<SearchNotesByContent.Note>
  )
}
