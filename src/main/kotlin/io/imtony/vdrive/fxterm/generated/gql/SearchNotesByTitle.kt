...
const val SEARCH_NOTES_BY_TITLE: String =
    "query SearchNotesByTitle(${'$'}text: String!, ${'$'}ignoreCase: Boolean, ${'$'}exact: Boolean) {\r\n  findByTitle(title: ${'$'}text, ignoreCase: ${'$'}ignoreCase, exact: ${'$'}exact) {\r\n    id\r\n    title\r\n    content\r\n    tags\r\n    created\r\n    updated\r\n  }\r\n}"

class SearchNotesByTitle(
  private val graphQLClient: GraphQLWebClient
) {
  suspend fun execute(variables: SearchNotesByTitle.Variables,
      requestBuilder: WebClient.RequestBodyUriSpec.() -> Unit = {}):
      GraphQLResponse<SearchNotesByTitle.Result> = graphQLClient.execute(SEARCH_NOTES_BY_TITLE,
      "SearchNotesByTitle", variables, requestBuilder)
  ...
  data class Note(
     ...
     */
    val id: SearchNotesByTitle.UUID,
    /**
     ...
     */
    val created: SearchNotesByTitle.LocalDateTime,
    /**
     * The LocalDateTime of when the Note was last updated.
     */
    val updated: SearchNotesByTitle.LocalDateTime
  )

  data class Result(
    val findByTitle: List<SearchNotesByTitle.Note>
  )
}
